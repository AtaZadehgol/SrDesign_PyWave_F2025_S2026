...
def find_alpha(sigma, Lc, eps_rel_bg, eps_rel_fg, f0):
    Be = 1
    eta0 = 120*np.pi
    n1 = np.sqrt(eps_rel_fg * eps_rel_bg)
    n2 = np.sqrt(eps_rel_bg)
    d = 100e-9
    k0 = 2 * np.pi * f0 / 3e8
...
def analysis(cfg: InitialValues_2DTE_Scattering):
    sigma_corners = [15]
    lc_corners = [700]
    ...
    for kind in range(4):
        ...
        for s in sigma_corners:
            ...
            for l in lc_corners:
                ...
                b = 0
                cfa = (1 - m*l - b)
                eps_rel_bg = cfg.eps_rel_bg
                eps_rel_fg = cfg.eps_rel_fg
                f0 = cfg.f0
                analytic = find_alpha(s*1e-9, l*1e-9, eps_rel_bg, eps_rel_fg, f0)[kind] * cfa

...
# this is the old config file that users manually edited.
# left for documentation, but NOT IN USE
...
# Geometry
sgl_wg_length = 45.0e-6        # Waveguide length (m, float)
sgl_wg_width = 200e-9          # Waveguide width (m, float)
port1_length = 5e-6            # Mode settling length for port 1 (m, float)
port2_length = 0e-6            # Mode settling length for port 2 (m, float)
...
# Directories
output_dir = 'Results'         # File output subfolder name

# Output files
hz_fft_name = 'hz_fft'         # File name for single frequency Hz results
ex_fft_name = 'ex_fft'         # File name for single frequency Ex results
ey_fft_name = 'ey_fft'         # File name for single frequency Ey results

profile_number = 0             # Simulation ID for batch simulation
...
source_type = 1                # Input source type [st] (, int)
                               ...
                               #     2: Wave packet
                               #     3: Time-harmonic
source_amp = 20.0              # Source magnitude (A / m, float)
wave_packet_bw = 0.10          # Wave-packet bandwidth; may be safely ignored when not using st 2 (%, float)
...
buffer_xhat = 20               # Additional buffer between souce point and cpml region, x-direction (cells, int)
...
buffer_yhat = int(ny_clad_wl*wl_clad/delta_x)    # Cladding buffer between waveguide and cpml region, y-direction (cells, int)
...
sx = bx + 10                                     # Source point, x-direction
...
#no magic numbers - iterative calculations
...
p1x = sx + int(port1_length/delta_x)
p2x = sx + nx_swg - int(port2_length/delta_x)
if p2x <= p1x:

...
class InitialValues_2DTE_Wave:
    def __init__(self):
        self.ready = False

        #brian defaults
        self.eps_rel_bg = 1.5**2
        self.mu_rel_bg = 1.0           # added. brian just always assumed 1.0 (non-magnetic)

        # below 9 are for if there's just one rectangle
        self.eps_rel_fg = 3.5**2  
        self.x_interior_region = 25.0e-6        # x dimension of interior region (if legacy single waveguide, this is x dim of waveguide)
        self.y_interior_region = 200e-9         # y dimension of interior region (if legacy single waveguide, this is y dim of waveguide)
        ...
        # port lengths are moot now! TODO: remove
        self.port1_length = 0e-6
        self.port2_length = 0e-6

        #per source, but leaving global options to default to
        self.source_type = 1
        ...
        self.points_per_wl = 40         #old way to set up delta x
        self.num_cpml = 20
    ...
    # Physical constants and value normalization
        ...
        print(self.flight_time, self.x_interior_region, self.num_flights)
        ...
        self.bx = self.__num_cpml + self.buffer_cells                     # Border size, x-direction (cells)  #was + self.__buffer_xhat 
        self.by = self.__num_cpml + self.buffer_cells                     # Border size, y-direction (cells)  #was + self.buffer_yhat 
        ...
        #should be able to change these -- but really want to allow nx_swg and ny_swg to change
    ...
    def expand_measurement_points(self):
        ...
        - 'x', 'y': position in meters
        """
        if not hasattr(self, '_InitialValues_2DTE_Wave__measurement_pts') or not self.__measurement_pts:
            return []
        
        measurement_regions = []
        print('in expand meas')

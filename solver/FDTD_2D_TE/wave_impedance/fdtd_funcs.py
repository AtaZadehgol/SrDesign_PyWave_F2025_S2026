...
# below is old, boolean style for only one waveguide
...
# grid initializer for multiple waveguides - NO ROUGHNESS CAPABILITY
def gen_eps_map_from_rectangles(nx, ny, rectangles, bx, by, delta_x, eps_rel_bg):
    """
    ...
    delta_x : float
        Grid spacing in meters (from FDTD solver, not GUI)
    eps_rel_bg : float
        Background relative permittivity (already squared, e.g., 1.5**2)
    ...
    for rect in rectangles:
        ...
        # Convert from meters to grid cell indices
        x_start = int(pos_x_meters / delta_x)
        y_start = int(pos_y_meters / delta_x)
        x_end = int((pos_x_meters + width_meters) / delta_x)
        y_end = int((pos_y_meters + height_meters) / delta_x)

        # Add border offset to get absolute positions in full grid
        x_start_interior = bx + x_start_interior
        y_start_interior = by + y_start_interior
        x_end_interior = bx + x_end_interior
        y_end_interior = by + y_end_interior
...
def gen_eps_map_from_rectangles_textured(nx, ny, rectangles, delta_x, eps_rel_bg, bx, by,
    ...
    for rect_idx, rect in enumerate(rectangles):
        ...
        if rough_enabled and rect_width_cells > 0 and rect_height_cells > 0:
            ...
            print(width_meters, rough_std, rough_acl, max_acl)
            print(rect_width_cells)

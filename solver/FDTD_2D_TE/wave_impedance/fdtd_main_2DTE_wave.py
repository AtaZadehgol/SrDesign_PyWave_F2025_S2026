...
def solver(cfg: InitialValues_2DTE_Wave):
    ...
    if hasattr(cfg, 'rectangles') and len(cfg.rectangles) > 0:
        ...
        #print(f"Should see: 1.0 (background) and {4.0/cfg.eps_rel_bg:.3f}, {12.25/cfg.eps_rel_bg:.3f} (rectangles)")
    ...
    HZT = np.zeros([cfg.nt, cfg.ny], dtype=np.float32)
    EXT = np.zeros([cfg.nt, cfg.ny], dtype=np.float32)
    ...
    # Oddly sized device arrays
    dHZT = cuda.to_device(HZT)
    dEXT = cuda.to_device(EXT)
    ...
    if measurement_regions is not None:
        ...
        for idx, region in enumerate(measurement_regions):
            ...
            device_region = {
                ...
                'dEX': cuda.to_device(mdata['EX']),
                'dEY': cuda.to_device(mdata['EY']),
                'dHZ': cuda.to_device(mdata['HZ']),
            ...
            print(f"  {region['name']} ({region['type']}): {region['num_points']} points, bpg={bpg_region}; grid position ({src_x}, {src_y})")
    ...
    for n in range(cfg.nt):
        ...
        # Record Data
        funcs.map_efield_zwave[bpgy, tpby](dEXT, dEX, n, cfg.cx, cfg.ny)
        funcs.map_hfield_zwave[bpgy, tpby](dHZT, dHZ, n, cfg.cx, cfg.ny)
    ...
    np.save('{}/ex_zwave'.format(cfg.output_dir), dEXT.copy_to_host(EXT))
    np.save('{}/hz_zwave'.format(cfg.output_dir), dHZT.copy_to_host(HZT))

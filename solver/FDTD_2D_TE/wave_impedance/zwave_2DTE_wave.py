...
#old for only one set of output files
def zwave(cfg: InitialValues_2DTE_Wave):
    cell = 3
    yref = cfg.by - cell

    ex_td = np.load(f'{cfg.output_dir}/ex_zwave.npy')[:, yref]
    hz_td = np.load(f'{cfg.output_dir}/hz_zwave.npy')[:, yref]
...
def zwave_analysis(cfg: InitialValues_2DTE_Wave, measurement_name=None, point_index=None):
    """
    Wave impedance analysis for measurement regions.
    
    Parameters:
    -----------
    cfg : InitialValues_2DTE_Wave
    ...
    if measurement_name is not None:
        ...
        hz_file = f'{cfg.output_dir}/hz_{safe_name}.npy'
        ...
        hz_data = np.load(hz_file)  # Shape: [nt, num_points]
...
def analyze_all_measurements(cfg: InitialValues_2DTE_Wave):
    """
...
# Convenience wrapper matching old function name
def zwave(cfg: InitialValues_2DTE_Wave):
    """
    Legacy interface - analyzes all measurements or falls back to legacy mode.
    """
    analyze_all_measurements(cfg)

...
class InitialValues_2DTM_Scattering:
    def __init__(self):
        self.ready = False
        ...
        #below for 1 rectangle
        self.eps_rel_fg = 3.5**2
        self.port_length = 0e-6
        self.x_interior_region = 30e-6          # interior x dimensions (legacy single wg length)
        self.y_interior_region = 200e-9         # interior x dimensions (legacy single wg width)
        ...
        self.ez_fft_name = 'ez_fft'
        self.hx_fft_name = 'hx_fft'
        self.hy_fft_name = 'hy_fft'
    ...
    #output files
    @property
    def ez_fft_name(self):
        return self.__ez_fft_name
    @ez_fft_name.setter
    def ez_fft_name(self, ez_fft_name):
        self.__ez_fft_name = ez_fft_name

    @property
    def hx_fft_name(self):
        return self.__hx_fft_name
    @hx_fft_name.setter
    def hx_fft_name(self, hx_fft_name):
        self.__hx_fft_name = hx_fft_name

    @property
    def hy_fft_name(self):
        return self.__hy_fft_name
    @hy_fft_name.setter
    def hy_fft_name(self, hy_fft_name):
        self.__hy_fft_name = hy_fft_name
    ...
    def automatedVarInit(self):
        ...
        self.cpml_range = self.num_cpml + 1                      # Total CPML layers, including PEC boundary (cells)

        self.nt = int( self.sim_time / self.delta_t )                     # Total simulation duration (steps)
        self.nx_swg = int( self.__x_interior_region / self.delta_x )      # Single waveguide size, x-direction (cells)
        self.ny_swg = int( self.__y_interior_region / self.delta_x )      # Single waveguide size, y-direction (cells)
        self.bx = self.__num_cpml + self.buffer_cells                     # Border size, x-direction (cells)
        self.by = self.__num_cpml + self.buffer_cells                     # Border size, y-direction (cells)
        ...
        #should be able to change these     - but really allowing user to change nx_swg and ny_swg (interior region)
        ...
        # Relational parameters - not used
        ...
        # Port Settling - not used
        self.p1x = self.sx + int(self.__port_length/self.delta_x)
        self.p2x = self.nx - self.p1x
    ...
    def expand_measurement_points(self):
        ...
        - 'x', 'y': position in meters
        """
        if not hasattr(self, '_InitialValues_2DTM_Scattering__measurement_pts') or not self.__measurement_pts:
            return []

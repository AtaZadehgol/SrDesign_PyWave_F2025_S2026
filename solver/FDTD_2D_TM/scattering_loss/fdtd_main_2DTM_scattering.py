...
def solver(cfg: InitialValues_2DTM_Scattering):
    ...
    EZfr = np.zeros([cfg.nx, cfg.ny], dtype=np.float32)
    EZfi = np.zeros([cfg.nx, cfg.ny], dtype=np.float32)
    HXfr = np.zeros([cfg.nx, cfg.ny], dtype=np.float32)
    HXfi = np.zeros([cfg.nx, cfg.ny], dtype=np.float32)
    HYfr = np.zeros([cfg.nx, cfg.ny], dtype=np.float32)
    HYfi = np.zeros([cfg.nx, cfg.ny], dtype=np.float32)
        ...
        if len(measurement_regions) > 0:
            ...
            for region in measurement_regions:
                ...
                region_data = {
                    ...
                    'EZ_real': np.zeros([num_pts], dtype=np.float32),
                    'EZ_imag': np.zeros([num_pts], dtype=np.float32),
                    'HX_real': np.zeros([num_pts], dtype=np.float32),
                    'HX_imag': np.zeros([num_pts], dtype=np.float32),
                    'HY_real': np.zeros([num_pts], dtype=np.float32),
                    'HY_imag': np.zeros([num_pts], dtype=np.float32),
                }
    ...
    cells_on_device = 5*(cfg.nx*cfg.ny) + 4*(cfg.cpml_range + cfg.cpml_range*cfg.nx + cfg.cpml_range*cfg.ny) + 2*(cfg.nx + cfg.nt*cfg.ny) + 6*(cfg.ny)
    ...
    dEZfr = cuda.to_device(EZfr)
    dEZfi = cuda.to_device(EZfi)
    dHXfr = cuda.to_device(HXfr)
    dHXfi = cuda.to_device(HXfi)
    dHYfr = cuda.to_device(HYfr)
    dHYfi = cuda.to_device(HYfi)
    ...
    if measurement_regions is not None:
        ...
        for idx, region in enumerate(measurement_regions):
            ...
            device_region = {
                ...
                'dEZ_real': cuda.to_device(mdata['EZ_real']),
                'dEZ_imag': cuda.to_device(mdata['EZ_imag']),
                'dHX_real': cuda.to_device(mdata['HX_real']),
                'dHX_imag': cuda.to_device(mdata['HX_imag']),
                'dHY_real': cuda.to_device(mdata['HY_real']),
                'dHY_imag': cuda.to_device(mdata['HY_imag']),
    ...
    for n in range(cfg.nt):
        ...
        # Record data
        funcs.simul_fft[bpg, tpb](dEZfr, dEZfi, dEZ, cfg.f0, n+0.5, cfg.delta_t, cfg.nx, cfg.ny)
        funcs.simul_fft[bpg, tpb](dHXfr, dHXfi, dHX, cfg.f0, n+1.0, cfg.delta_t, cfg.nx, cfg.ny)
        funcs.simul_fft[bpg, tpb](dHYfr, dHYfi, dHY, cfg.f0, n+1.0, cfg.delta_t, cfg.nx, cfg.ny)
        ...
        if device_measurement_data is not None:
            for region_data in device_measurement_data:
                funcs.simul_fft_points[region_data['bpg'], tpby](
                    region_data['dEZ_real'], region_data['dEZ_imag'], dEZ, 
                    region_data['dx_indices'], region_data['dy_indices'],
                    cfg.f0, n+0.5, cfg.delta_t, region_data['num_points']
                )
                funcs.simul_fft_points[region_data['bpg'], tpby](
                    region_data['dHX_real'], region_data['dHX_imag'], dHX,
                    region_data['dx_indices'], region_data['dy_indices'],
                    cfg.f0, n+0.5, cfg.delta_t, region_data['num_points']
                )
                funcs.simul_fft_points[region_data['bpg'], tpby](
                    region_data['dHY_real'], region_data['dHY_imag'], dHY,
                    region_data['dx_indices'], region_data['dy_indices'],
    ...
    #np.save(cfg.output_dir+'/'+cfg.ez_fft_name+cfg.roughness_profile, dEZfr.copy_to_host() - 1j*dEZfi.copy_to_host())
    #np.save(cfg.output_dir+'/'+cfg.hx_fft_name+cfg.roughness_profile, dHXfr.copy_to_host() - 1j*dHXfi.copy_to_host())
    #np.save(cfg.output_dir+'/'+cfg.hy_fft_name+cfg.roughness_profile, dHYfr.copy_to_host() - 1j*dHYfi.copy_to_host())
    np.save('{}/ez_zwave'.format(cfg.output_dir), dEZfr.copy_to_host() - 1j*dEZfi.copy_to_host())
    np.save('{}/hx_zwave'.format(cfg.output_dir), dHXfr.copy_to_host() - 1j*dHXfi.copy_to_host())
    np.save('{}/hy_zwave'.format(cfg.output_dir), dHYfr.copy_to_host() - 1j*dHYfi.copy_to_host())
    ...
    if device_measurement_data is not None:
        ...
        for idx, region_data in enumerate(device_measurement_data):
            ...
            ez_complex = (region_data['dEZ_real'].copy_to_host(mdata['EZ_real']) - 
                        j*region_data['dEZ_imag'].copy_to_host(mdata['EZ_imag']))
            hx_complex = (region_data['dHX_real'].copy_to_host(mdata['HX_real']) - 
                        j*region_data['dHX_imag'].copy_to_host(mdata['HX_imag']))
            hy_complex = (region_data['dHY_real'].copy_to_host(mdata['HY_real']) - 
                        j*region_data['dHY_imag'].copy_to_host(mdata['HY_imag']))
            
            # Save frequency-domain fields (shape: [num_points])
            np.save(f'{cfg.output_dir}/ez_{safe_name}_fft.npy', ez_complex)
            np.save(f'{cfg.output_dir}/hx_{safe_name}_fft.npy', hx_complex)
            np.save(f'{cfg.output_dir}/hy_{safe_name}_fft.npy', hy_complex)

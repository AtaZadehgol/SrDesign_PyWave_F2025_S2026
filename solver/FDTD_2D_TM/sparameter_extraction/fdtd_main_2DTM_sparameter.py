...
def solver(cfg: InitialValues_2DTM_SParameter):
    ...
    EZ = np.zeros([cfg.nx, cfg.ny], dtype=cfg.precision)     # Electric field at current time step, z-component
    HX = np.zeros([cfg.nx, cfg.ny], dtype=cfg.precision)     # Magnetic field at current time step, x-component
    HY = np.zeros([cfg.nx, cfg.ny], dtype=cfg.precision)     # Magnetic field at current time step, y-component

    # Assign Materials
    if cfg.sim_type == 's-param':
        # Get sim input
        args = sys.argv
        ...
        if cfg.rough_toggle:
            if kind == 'i':
                ...
                FG_REG = funcs.generate_fg_mask(cfg.nx, cfg.ny, cfg.bx, cfg.by, cfg.ny_swg, cfg.pp, cfg.num_lines, cfg.p1x, cfg.nx-cfg.p2x, cfg.rstd, cfg.racl, cfg.delta_x, mode='smooth', out_dir=cfg.prof_dir)
            elif line == 0 and direction == 'f' and kind == 'r':
                # Make new roughness profiles during first line, first reflected sim
                FG_REG = funcs.generate_fg_mask(cfg.nx, cfg.ny, cfg.bx, cfg.by, cfg.ny_swg, cfg.pp, cfg.num_lines, cfg.p1x, cfg.nx-cfg.p2x, cfg.rstd, cfg.racl, cfg.delta_x, out_dir=cfg.prof_dir, upper_name=cfg.up, lower_name=cfg.lp, atol=cfg.tol_acl, stol=cfg.tol_std, mtol=0.01)
            else:
                # Load the roughness mask for all other sim types. This conditional hierarchy should make these only during reflected sims
                FG_REG = funcs.generate_fg_mask(cfg.nx, cfg.ny, cfg.bx, cfg.by, cfg.ny_swg, cfg.pp, cfg.num_lines, cfg.p1x, cfg.nx-cfg.p2x, cfg.rstd, cfg.racl, cfg.delta_x, mode='load', out_dir=cfg.prof_dir, upper_name=cfg.up+'.npy', lower_name=cfg.lp+'.npy', atol=cfg.tol_acl, stol=cfg.tol_std, mtol=0.01)
        else:
    ...
    EPS_EZ = EPS_MASK.copy().astype(cfg.precision)
    CBZ = cfg.delta_t / EPS_EZ
    DB = cfg.delta_t / cfg.mu

    cosE = np.zeros([cfg.sparam_num_freqs, cfg.nt], dtype=cfg.precision)
    cosH = np.zeros([cfg.sparam_num_freqs, cfg.nt], dtype=cfg.precision)
    sinE = np.zeros([cfg.sparam_num_freqs, cfg.nt], dtype=cfg.precision)
    sinH = np.zeros([cfg.sparam_num_freqs, cfg.nt], dtype=cfg.precision)

    funcs.map_sfft_coeffs(cosE, sinE, cosH, sinH, cfg.sparam_freqs, cfg.delta_t, cfg.nt)

    # Define data storage arrays
    EZ1r = np.zeros([cfg.sparam_num_freqs, cfg.ny], dtype=cfg.precision)
    HY1r = np.zeros([cfg.sparam_num_freqs, cfg.ny], dtype=cfg.precision)

    EZ1i = np.zeros([cfg.sparam_num_freqs, cfg.ny], dtype=cfg.precision)
    HY1i = np.zeros([cfg.sparam_num_freqs, cfg.ny], dtype=cfg.precision)

    EZ2r = np.zeros([cfg.sparam_num_freqs, cfg.ny], dtype=cfg.precision)
    HY2r = np.zeros([cfg.sparam_num_freqs, cfg.ny], dtype=cfg.precision)

    EZ2i = np.zeros([cfg.sparam_num_freqs, cfg.ny], dtype=cfg.precision)
    HY2i = np.zeros([cfg.sparam_num_freqs, cfg.ny], dtype=cfg.precision)
    ...
    # Define arrays
    PSI_EZ_XLO = np.zeros([cfg.num_cpml, cfg.ny], dtype=cfg.precision)  # Ez correction field, low-side x-boundary
    PSI_EZ_XHI = np.zeros([cfg.num_cpml, cfg.ny], dtype=cfg.precision)  # Ez correction field, high-side x-boundary
    PSI_EZ_YLO = np.zeros([cfg.nx, cfg.num_cpml], dtype=cfg.precision)  # Ez correction field, low-side y-boundary
    PSI_EZ_YHI = np.zeros([cfg.nx, cfg.num_cpml], dtype=cfg.precision)  # Ez correction field, high-side y-boundary

    PSI_HX_YLO = np.zeros([cfg.nx, cfg.num_cpml], dtype=cfg.precision)  # Hx correction field, low-side y-boundary
    PSI_HX_YHI = np.zeros([cfg.nx, cfg.num_cpml], dtype=cfg.precision)  # Hx correction field, high-side y-boundary

    PSI_HY_XLO = np.zeros([cfg.num_cpml, cfg.ny], dtype=cfg.precision)  # Hy correction field, low-side x-boundary
    PSI_HY_XHI = np.zeros([cfg.num_cpml, cfg.ny], dtype=cfg.precision)  # Hy correction field, high-side x-boundary

    SE = np.zeros(cfg.num_cpml, dtype=cfg.precision)                    # Electric field sigma grading
    KE = np.zeros(cfg.num_cpml, dtype=cfg.precision)                    # Electric field kappa grading
    AE = np.zeros(cfg.num_cpml, dtype=cfg.precision)                    # Electric field alpha grading
    BE = np.zeros(cfg.num_cpml, dtype=cfg.precision)                    # Electric field auxiliary variable b
    CE = np.zeros(cfg.num_cpml, dtype=cfg.precision)                    # Electric field auxiliary variable c

    SH = np.zeros(cfg.num_cpml, dtype=cfg.precision)                    # Magnetic field sigma grading
    KH = np.zeros(cfg.num_cpml, dtype=cfg.precision)                    # Magnetic field kappa grading
    AH = np.zeros(cfg.num_cpml, dtype=cfg.precision)                    # Magnetic field alpha grading
    BH = np.zeros(cfg.num_cpml, dtype=cfg.precision)                    # Magnetic field auxiliary variable b
    CH = np.zeros(cfg.num_cpml, dtype=cfg.precision)                    # Magnetic field auxiliary variable c

    KXE = np.ones(cfg.nx, dtype=cfg.precision) * cfg.delta_x               # Electric field x-direction kappa division
    KYE = np.ones(cfg.ny, dtype=cfg.precision) * cfg.delta_x               # Electric field y-direction kappa division
    KXH = np.ones(cfg.nx, dtype=cfg.precision) * cfg.delta_x               # Magnetic field x-direction kappa division
    KYH = np.ones(cfg.ny, dtype=cfg.precision) * cfg.delta_x               # Magnetic field y-direction kappa division
    ...
    tpb_sfft = (1, 128)
    bpg_sfft_f = int(np.ceil(cfg.sparam_num_freqs / tpb_sfft[0]))
    bpg_sfft_y = int(np.ceil(cfg.ny / tpb_sfft[1]))

    bpg_sfft = (bpg_sfft_f, bpg_sfft_y)

    # GPU process feedbak
    cells_on_device = 10*(cfg.nx * cfg.ny) + 4*cfg.num_cpml*(cfg.nx + cfg.ny + 1) + 2*(cfg.nx + cfg.ny)
    ...
    nfb = int( 100 / cfg.feedback_at_n_percent) + 1
    ...
    for n in range(cfg.nt):
        ...
        funcs.update_pml_ez_xinc[bpg_py, tpb](dPSI_EZ_XLO, dPSI_EZ_XHI, dEZ, dHY, dBE, dCE, dCBZ, cfg.delta_x, cfg.num_cpml, ib, jb, ie, je)
        funcs.update_pml_ez_yinc[bpg_xp, tpb](dPSI_EZ_YLO, dPSI_EZ_YHI, dEZ, dHX, dBE, dCE, dCBZ, cfg.delta_x, cfg.num_cpml, ib, jb, ie, je)
        ...
        funcs.update_pml_hx_yinc[bpg_xp, tpb](dPSI_HX_YLO, dPSI_HX_YHI, dEZ, dHX, dBH, dCH, DB, cfg.delta_x, cfg.num_cpml, ib, jb, ie, je)
        funcs.update_pml_hy_xinc[bpg_py, tpb](dPSI_HY_XLO, dPSI_HY_XHI, dEZ, dHY, dBH, dCH, DB, cfg.delta_x, cfg.num_cpml, ib, jb, ie, je)

        # Record Data
        funcs.simul_fft_efield[bpg_sfft, tpb_sfft](dEZ1r, dEZ1i, dEZ, dcosE, dsinE, n, cfg.sparam_num_freqs, cfg.p1x, cfg.num_cpml, cfg.ny-cfg.num_cpml)
        funcs.simul_fft_hfield[bpg_sfft, tpb_sfft](dHY1r, dHY1i, dHY, dcosH, dsinH, n, cfg.sparam_num_freqs, cfg.p1x, cfg.num_cpml, cfg.ny-cfg.num_cpml)
        funcs.simul_fft_efield[bpg_sfft, tpb_sfft](dEZ2r, dEZ2i, dEZ, dcosE, dsinE, n, cfg.sparam_num_freqs, cfg.p2x, cfg.num_cpml, cfg.ny-cfg.num_cpml)
        funcs.simul_fft_hfield[bpg_sfft, tpb_sfft](dHY2r, dHY2i, dHY, dcosH, dsinH, n, cfg.sparam_num_freqs, cfg.p2x, cfg.num_cpml, cfg.ny-cfg.num_cpml)
    ...
    if cfg.sim_type == 's-param':
        if kind == 'i':
            if direction == 'f':
                np.save('{}/ez_fi_l{}'.format(cfg.output_dir, line+1), EZF1)
                np.save('{}/hy_fi_l{}'.format(cfg.output_dir, line+1), HYF1)
            elif direction == 'b':
                np.save('{}/ez_bi_l{}'.format(cfg.output_dir, line+1), EZF2)
                np.save('{}/hy_bi_l{}'.format(cfg.output_dir, line+1), HYF2)
        elif kind == 'r':
            if direction == 'f':
                np.save('{}/ez_ft_l{}'.format(cfg.output_dir, line+1), EZF1)
                np.save('{}/hy_ft_l{}'.format(cfg.output_dir, line+1), HYF1)
                np.save('{}/ez_br_l{}'.format(cfg.output_dir, line+1), EZF2)
                np.save('{}/hy_br_l{}'.format(cfg.output_dir, line+1), HYF2)
            elif direction == 'b':
                np.save('{}/ez_fr_l{}'.format(cfg.output_dir, line+1), EZF1)
                np.save('{}/hy_fr_l{}'.format(cfg.output_dir, line+1), HYF1)
                np.save('{}/ez_bt_l{}'.format(cfg.output_dir, line+1), EZF2)
                np.save('{}/hy_bt_l{}'.format(cfg.output_dir, line+1), HYF2)
    else:
        pass

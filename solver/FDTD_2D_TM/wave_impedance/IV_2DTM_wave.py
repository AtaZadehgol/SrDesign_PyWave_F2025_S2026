...
class InitialValues_2DTM_Wave:
    def __init__(self):
        ...
        #below 9 are for if there's just one waveguide (rectangle)
        self.eps_rel_fg = 3.5**2
        self.x_interior_region = 30e-6
        self.y_interior_region = 200e-9
        ...
        #port length is moot now! TODO: remove
        self.port_length = 0e-6

        #below are per source, but leaving global options as defaults
        self.source_type = 1
        self.source_amp = 5.0
        ...
        self.points_per_wl = 40         # old way to calculate delta x
        self.num_cpml = 20

        self.delta_x = -1               # Set to -1 for now - will either be set in config or recalculated
        self.delta_t_coef = 1

        #below 2 are old ways to set up buffer
        self.buffer_xhat = 20
        self.ny_clad_wl = 2.0 

        self.buffer_wavelengths = 0 #2.0   #new way
    ...
    def automatedVarInit(self):
        ...
        self.mu = self.__mu_rel_bg * 4.0e-7 * np.pi                                        # Base permeability (H/m)   #mu knot
        ...
        self.delta_t = self.delta_t_coef * self.__delta_x / ( self.c_max * np.sqrt( 2 ) )    # Temporal resolution (s/step)
        ...
        self.buffer_cells = int(self.__buffer_wavelengths * self.wl_clad / self.delta_x)
        ...
        self.nx_swg = int( self.__x_interior_region / self.delta_x )    # inteior region, x-direction (cells)
        self.ny_swg = int( self.__y_interior_region / self.delta_x )    # interior, y-direction (cells)
    ...
    def expand_measurement_points(self):
        ...
        - 'x', 'y': position in meters
        """
        if not hasattr(self, '_InitialValues_2DTM_Wave__measurement_pts') or not self.__measurement_pts:
            return []

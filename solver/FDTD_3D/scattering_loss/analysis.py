...
results_file_name = 'results_3D_TE.npy'  # Output file name from automated_results_collection.py
mode = 'TE'                              # Analysis mode type, pick 'TE' or 'TM'
sigmas = [15]                            # Analysis sigma values, pick only values with at least 1 result (nm)
lcs = [700]                              # Analysis Lc values, pick only values with at least 1 result (nm)
...
fdtd = np.load('{}/{}'.format(cfg.output_dir, results_file_name))
if mode == 'TE':
    kind_lim = 2
elif mode == 'TM':
    kind_lim = 4
for kind in range(kind_lim):
    if kind == 0:
        if mode == 'TE':
    ...
    for s in sigmas:
        ...
        for l in lcs:
            ...
            set_sigma = np.mean(this_set[3])
            set_lc = np.mean(this_set[4])
            set_alpha = this_set[2]
            if mode == 'TE':
                analytic = find_alpha_te(s*1e-9, l*1e-9)[kind]
            elif mode == 'TM':
                analytic = find_alpha_tm(s*1e-9, l*1e-9)[kind]
            ...
            print('Lc: {:} nm,\t samples: {:},\t set sd: {:.0f},\t set mean: {:.0f} Np/m,\t BW: +/-{:.3f}%,\terror: {:.2f}%,\tmin.e: {:.2f}%\tmax.e: {:.2f}%'.format(l, this_set.shape[1], np.std(set_alpha), np.mean(set_alpha), 100*np.std(set_alpha)/np.mean(set_alpha), error, eplus, eminus))
        print('Deviation cf across LCs for this sigma is:\t{:.3f}\n'.format(np.std(sigma_offsets)))

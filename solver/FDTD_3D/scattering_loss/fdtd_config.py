...
#Version 1.0 success:
#can draw rectangle wave guide(s) - could be parallel in multiple dimensions
#can specify source anywhere in 3d space (on, in, outside wave geuides)
#measurement point anywhere
#would really like surface roughness on side walls, and allow user to control that
#   happy with even just the autocorrelation length, height of surface roughness (sigma) (paper by lacey and pain)
#is still currently only modeled along 1D. he takes it and applies to 2D and 3D (missing those additional variations tho)
#just repeated in the other dimensions. works 'ok' for 2D but certainly off in 3d
...
# Geometry
sgl_wg_length = 24.0e-6        # Waveguide length (m, float)
sgl_wg_width = 2.0e-6          # Single waveguide width (m, float) MAKE THIS WIDER!!!
sgl_wg_height = 0.2e-6         # Single waveguide height (m, float)
port1_length = 4.0e-6          # Modal settling length for port 1 (m, float)
port2_length = 0.0e-6          # Modal settling length for port 2 (m, float)

# Roughness
rough_std = 15.0e-9            # Target roughness standard deviation (m, float) #standard deviation (sigma)
rough_acl = 700.0e-9           # Target roughness correlation length (m, float) #autocorrelation length
...
# Output files
ex1_fft_name = 'ex1_fft'
ey1_fft_name = 'ey1_fft'
ez1_fft_name = 'ez1_fft'
hx1_fft_name = 'hx1_fft'
hy1_fft_name = 'hy1_fft'
hz1_fft_name = 'hz1_fft'
ex2_fft_name = 'ex2_fft'
ey2_fft_name = 'ey2_fft'
ez2_fft_name = 'ez2_fft'
hx2_fft_name = 'hx2_fft'
hy2_fft_name = 'hy2_fft'
hz2_fft_name = 'hz2_fft'
...
points_per_wl = 30             # Number of points per minimum wavelength (cells, int)

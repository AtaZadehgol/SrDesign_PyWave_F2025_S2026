...
#same, delta_t = V * delta_x - give them the option to change V (including sqrt) but set default as this
#will want some limits, checking
...
#should be variables - scale should be their option
...
for n in range(nt):
    cosE[n] = np.cos( 2 * np.pi * f0 * (n+0.0) * delta_t, dtype=np.float32) #fixed!!
